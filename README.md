...
- **Dificultad**: rampa de spawns más agresiva (`interval_shrink_per_minute` 0.25, `extra_count_per_minute` 0.35), más shiny (6% → 16%), y un **escalado tardío abierto** desde el minuto 6 (+6% HP y +4% daño por minuto en spawns nuevos) para que el late game no se pueda ignorar quieto. En co-op los enemigos escalan por jugador extra (+50% HP, +30% ritmo de spawn, jefes +50% HP por jugador).
