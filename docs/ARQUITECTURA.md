...
- **Knobs de partida sin consumidor (ganchos hacia adelante)**: `RunState.disaster_chance_bonus` lo escriben los pactos demoníacos y **nadie lo lee todavía** — es el gancho de los desastres de la parte C, igual que `demonic_uses` fue el gancho de la iteración 42. Se reinicia en `reset()` como los demás. No lo borres por «dato muerto»: un pacto ya lo vende.
