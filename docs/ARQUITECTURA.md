...
| `hud` | el HUD | `announce(msg)`, `announce_major(msg)` |
| `boss_ui` | HUD + flecha de jefe | `track_boss(boss, title)`, `announce` |
| `upgrade_ui` | `UpgradeCardUI` | `open_bonus_pick(...)` |
...
- Hooks a sobreescribir: `_behavior_tick(delta)` (cooldowns), `_movement_intent(seek, distance)` (a dónde ir), `_combat_tick(player, distance)` (atacar), `_facing_direction(...)` y **`_apply_elite_damage(mult)`** (multiplica los números de daño propios — lo llaman élites y `apply_tier_scaling(hp, dmg, xp)`).
...
- Overlay de material: hay **un solo** punto de verdad con precedencia élite > variante > estado (`current_overlay()`). Un efecto que pinte al enemigo pasa por ahí; escribir `material_overlay` a mano borra el tinte de otro sistema (era el bug del veneno que borraba la variante).
...
- `spawn_chest(at, luck_bonus, min_rarity)` es el único camino a un cofre soltado, compartido por el cofre de élite (`elite_chest_chance × (1 + difficulty_bonus)`) y el anillo del jefe.
...
- Invocaciones: `call_group("enemy_spawner", "spawn_minions", escena, posiciones)`. Los esbirros pasan por **el mismo** escalado que un spawn de anillo (tier, rampa tardía, dificultad, buff temporal, HP de co-op, variante de cielo, tirada de élite). Instanciarlos a mano se saltaba todo eso.
...
- Qué es: economía por jugador dentro de la partida. Cada muerte paga `EnemyBase.points_value` (élites x5, jefes `BossBase.boss_points_value` = 25, escalado por `apply_tier`) al raider **más cercano** (`Player.add_points`, señal `points_changed`; se reinicia cada partida).
...
- Cofres: `rarity` fija en la instancia o tirada en `_ready` (pesos de rareza + `luck_bonus` de la party, piso `min_rarity`). El **id** de rareza y el **nombre mostrado** están separados. Precio = base por rareza × multiplicador global que crece x1.25 con **cada** cofre abierto. Log `Chest opened:`.
  - Decisión de diseño pendiente: la tirada de rareza sigue en `_ready` (no al abrir), porque rodar al abrir con piso = rareza del cofre equivale a tomar el máximo de dos tiradas e infla la economía, y además el tinte y el precio se fijan en `_ready` para que se lea a distancia. Si alguien lleva el diseño de la economía, es su decisión.
...
- Cartas (`upgrade_card_ui.gd`): en `RunState.leveled_up` pausa el árbol y ofrece 3 tiradas de `UpgradePool.roll_offer()`; los picks extra se encolan. **`open_bonus_pick(title, luck_bonus, min_rarity, recipient)`** lleva destinatario explícito: en co-op, un cofre o altar tiene que decir a **quién** le toca la carta, no confiar en «el jugador de turno».
